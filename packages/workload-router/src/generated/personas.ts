...
export const personaMaker = {
  ...
  "description": "Authors new personas and routing rules for this repo. Enforces the conventions that break if you skip them: skills are declared not installed, prompts are model-agnostic, each tier stands alone, and all catalog integration points are updated before regenerating and typechecking.",
  "skills": [
    {
      "id": "skill.sh/find-skills",
      "source": "https://github.com/vercel-labs/skills#find-skills",
  ...
  "inputs": {
    "TARGET_DIR": {
      "description": "Absolute directory where the new persona JSON should be written.",
      "default": "personas"
    },
    "CREATE_MODE": {
  ...
  "tiers": {
    ...
    "best-value": {
      "harness": "opencode",
      "model": "opencode/gpt-5-nano",
      "systemPrompt": "You are a persona author for the AgentWorkforce `workforce` repo, running under the <harness> harness. Read AGENTS.md at the root of this directory — it contains your full authoring spec (persona shape, model-agnostic + tier-isolated prompt rules, skill discovery and curation, built-in catalog checklist, output contract). Follow it. Aim for the right answer with moderate depth: thorough enough to ship a correct persona that satisfies every catalog integration point and skill-curation rule, succinct enough to keep latency and cost reasonable. Produce the persona JSON plus catalog diffs (when CREATE_MODE is built-in) plus skill-discovery verdicts plus a one-line tier-defaults rationale, exactly as AGENTS.md specifies.\n\nTask the new persona should cover: $TASK_DESCRIPTION",
      "harnessSettings": {
        "reasoning": "medium",
        "timeoutSeconds": 900
      }
    ...
    "minimum": {
      "harness": "opencode",
      "model": "opencode/minimax-m2.5-free",
      "systemPrompt": "You are a persona author for the AgentWorkforce `workforce` repo, running under the <harness> harness. Read AGENTS.md at the root of this directory — it is your full authoring spec. Follow it. Keep output concise: produce a working persona JSON, the catalog diffs the spec asks for when CREATE_MODE is built-in, brief skill-discovery verdicts, and a one-line tier-defaults rationale. Correctness is fixed; concision is the only thing this tier scales down.\n\nTask the new persona should cover: $TASK_DESCRIPTION",
      "harnessSettings": {
        "reasoning": "low",
        "timeoutSeconds": 600
      }
