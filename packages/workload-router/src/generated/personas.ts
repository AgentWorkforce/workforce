...
export const personaMaker = {
  ...
  "inputs": {
    ...
    "CREATE_MODE": {
      "description": "local|built-in: local writes JSON only; built-in also updates repo catalog/routing/test/docs. The CLI sets this to `built-in` only when the resolved target is the AgentWorkforce repo's `/personas` library (i.e. `--save-in-directory=library`); every other target is `local`.",
      "default": "local"
    },
    "TASK_DESCRIPTION": {
