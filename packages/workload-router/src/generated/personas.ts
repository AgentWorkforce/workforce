...
export const personaImprover = {
  ...
  "description": "Reviews a finished persona session and proposes concrete improvements (description, systemPrompt, skills, tier wiring) for the local persona file. Emits a structured JSON proposal list which the CLI walks one-by-one for user approval.",
  "inputs": {
    "PERSONA_FILE_PATH": {
      "description": "Absolute path to the local persona JSON file under review."
    },
    "SESSION_TRANSCRIPT_PATH": {
      "description": "Absolute path to the just-ended harness session transcript (typically a JSONL file). May be empty if the CLI could not locate one for the harness; in that case proceed from the persona file alone and note the limitation in the proposals.",
      "optional": true
    },
...
export const personaMaker = {
  ...
  "inputs": {
    "TARGET_DIR": {
      "description": "Absolute directory where the new persona JSON should be written. The CLI defaults this to `<cwd>/.agentworkforce/workforce/personas` (created if missing); override with `agentworkforce create --save-in-directory=<target>` or a saved `defaultCreateTarget` in `~/.agentworkforce/workforce/config.json`.",
    ...
    "TASK_DESCRIPTION": {
      "description": "The task or capability the new persona should cover. Auto-populated when launched from `agentworkforce pick` after no existing persona matched the user's request; otherwise omitted, in which case the agent reads its operating spec from AGENTS.md and waits for the user to describe a task in the TUI.",
      "optional": true
    }
  },
