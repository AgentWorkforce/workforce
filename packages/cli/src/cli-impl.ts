...
// Cloud-facing subcommand handlers (deploy/invoke/runs/destroy/integrations/
// deployments) are imported lazily at dispatch: their module graphs pull in
// @agent-relay/cloud and friends, which costs hundreds of milliseconds that
// the hot `agent <persona>` launch path must not pay.
import {
...
/**
 * Launch-phase timing marks, enabled with AGENTWORKFORCE_PERF=1. Each line is
 * `[perf] +<ms since process start> <label>` on stderr — cheap enough to keep
 * in the launch path permanently, and the only way to see where a slow
 * `agentworkforce agent <persona>` spends its time in the field.
 */
const PERF_ENABLED = process.env.AGENTWORKFORCE_PERF === '1';
  ...
  agent [flags] <persona>
                      ...
                      Flags:
                        --install-in-repo   Disengage the sandbox mount and
                                            ...
                                            (or AGENTS.md / skill roots for
                                            non-claude harnesses) are hidden
                                            from the session.
                        --no-launch-metadata
                      ...
                      mount is kept (under
                      ~/.agentworkforce/workforce/cache/warm-mounts/) along
                      with a validated launch plan, so the harness spawns in
                      tens of milliseconds and the mount reconciles with the
                      repo in the background while the harness boots. Any
                      change to the persona, its sources, skills, dotfiles,
                      or relevant environment falls back to the full launch.
                      Disable with AGENTWORKFORCE_NO_FAST=1.
 ...
 *
 * Used by the mount-harness branch to give non-claude harnesses the same
...
function mirrorSkillCacheInto(cacheDir: string, targetDir: string): void {
  ...
  for (const entry of readdirSync(cacheDir, { withFileTypes: true })) {
    ...
    // npm's download cache exists only so installs into the cache dir don't
    // re-fetch tarballs; it's dead weight at runtime (tens of MB) and the
    // mount excludes `.npm-cache` from sync entirely.
    if (entry.name === '.npm-cache') continue;
...
export const CLEAN_IGNORED_PATTERNS = [
  ...
  // running under a non-claude harness; without this the user's real-cwd AGENTS.md
...
export const SKILL_INSTALL_IGNORED_PATTERNS = [
  ...
  'prpm.lock',
  'skills-lock.json',
  // Per-persona AGENTS.md sidecars (non-claude harnesses) get materialized
...
/**
 * Build the warm-launch plan for this session, or null when the launch is
 * not eligible for the fast path.
 *
 * Eligibility is deliberately strict: the plan stores a fully resolved argv,
 * so anything whose value flowed in from the environment (persona `env`
 * maps, MCP server configs, relay MCP injection on harnesses that wire it,
 * env-provided persona inputs, ai-memory wiring) makes the launch ineligible
 * rather than risking a stale or secret-bearing plan on disk. Digests are
 * captured HERE — at resolution time — so an input file edited while the
 * session runs invalidates the plan instead of being blessed at exit.
 */
function buildFastLaunchPlan(input: {
  selector: string;
  personaId: string;
  harness: Harness;
  model: string;
  ...
  relayMcp: RelayMcpConfig | undefined;
  /** Resolution of the persona's declared `env` map (must be absent). */
  envResolutionValue: Record<string, string> | undefined;
  /** Input values from declared defaults — the only allowed env additions. */
  inputEnv: Record<string, string>;
  resolvedMcp: unknown;
  aiHist: unknown;
  skillCacheDir: string | undefined;
  skillCacheFingerprint: string | undefined;
  bin: string;
  ...
  if (input.envResolutionValue !== undefined) return null;
  if (input.resolvedMcp !== undefined) return null;
  if (input.aiHist !== undefined) return null;
  ...
  // MCP unset — enforced via the env pins below.
  if (input.relayMcp !== undefined && input.harness !== 'opencode') return null;

  const envExact: Record<string, string | null> = {};
  for (const name of Object.keys(input.selection.inputs ?? {})) {
    // Inputs must have resolved from declared defaults: an env-provided
    // value would bake user/session state (possibly secrets) into the plan.
    if (process.env[name] !== undefined) return null;
    envExact[name] = null;
  }
  envExact.AGENT_WORKFORCE_HOME = process.env.AGENT_WORKFORCE_HOME ?? null;
  envExact.AGENT_WORKFORCE_CONFIG_DIR = process.env.AGENT_WORKFORCE_CONFIG_DIR ?? null;
  if (input.harness !== 'opencode') {
    for (const name of [
      ...
      // relayMcp was undefined (checked above), so these were unset/blank;
      // pin them to "unset" without ever recording a value.
      if (process.env[name] !== undefined) return null;
      envExact[name] = null;
    }
  }

  const digests: StatDigest[] = [];
  const personaFilePath = local.paths.get(input.personaId);
  if (personaFilePath) digests.push(statDigestOf(personaFilePath));
  ...
  for (const name of [
    '.agentignore',
    '.agentreadonly',
    `.${input.personaId}.agentignore`,
    `.${input.personaId}.agentreadonly`
  ]) {
    digests.push(statDigestOf(join(cwd, name)));
  }
  // Local skill sources feed the cache fingerprint by content hash; the fast
  // path never recomputes the fingerprint, so pin the files themselves.
  for (const skill of input.effectiveSelection.skills) {
    if (SKILL_URL_PREFIX_RE.test(skill.source)) continue;
    if (!SKILL_LOCAL_MD_RE.test(skill.source)) continue;
    const trimmed = skill.source.startsWith('./') ? skill.source.slice(2) : skill.source;
    digests.push(statDigestOf(isAbsolute(trimmed) ? trimmed : join(cwd, trimmed)));
  }

  const binPath = resolveBinOnPath(input.bin);
  ...
  return {
    ...
    envExact,
    skillCache: {
      dir: input.skillCacheDir,
      fingerprint: input.skillCacheFingerprint,
      upstreamIntervalRaw: process.env.AGENTWORKFORCE_SKILL_CACHE_CHECK_INTERVAL ?? null,
      upstreamIntervalMs: resolveEnvCheckIntervalMs()
    },
    spawn: { binPath, args: [...input.finalArgs], envAdditions: { ...input.inputEnv } },
    mount: {
      warmDir: warmMountDir(cwd, input.selector),
      ignoredPatterns: [...input.ignoredPatterns],
      readonlyPatterns: [...input.readonlyPatterns]
    }
  };
}

/**
 * Keep a finished session's mount as the warm mount for the next launch and
 * persist the plan + autosync state next to it. Best-effort: any failure
 * leaves no warm dir (or a partial one whose missing marker/state file the
 * fast path rejects) and the next launch simply goes cold.
 */
function rewarmMountAndPlan(input: {
  plan: LaunchPlan;
  mountDir: string;
  exportedState: Record<string, FileState>;
}): void {
  const warmDir = input.plan.mount.warmDir;
  try {
    rmSync(warmDir, { recursive: true, force: true });
    mkdirSync(warmDir, { recursive: true });
    renameSync(input.mountDir, join(warmDir, 'mount'));
    writeFileSync(
      join(warmDir, 'state.json'),
      JSON.stringify(input.exportedState),
      { mode: 0o600 }
    );
    writeLaunchPlan(input.plan);
    perfMark('exit: mount re-warmed + plan written');
  } catch (err) {
    process.stderr.write(
      `warning: could not keep warm mount for next launch: ${(err as Error).message}\n`
    );
    try {
      rmSync(warmDir, { recursive: true, force: true });
    } catch {
...
/**
 * Interpolate declared persona inputs ($TARGET_DIR, ${CREATE_MODE}, …) into
 * the system prompt and sidecar content fields. Personas that move heavy
 * authoring guidance out of the systemPrompt and into AGENTS.md / CLAUDE.md
 * still want their declared inputs interpolated; without this the
 * materialized sidecar would carry literal `$TARGET_DIR` strings. Shared by
 * the full launch path and the fast-session resume (which re-renders to
 * verify the cached plan and to feed launch metadata).
 */
function renderEffectiveSelection(selection: PersonaSelection): {
  effectiveSelection: PersonaSelection;
  inputResolution: ReturnType<typeof resolvePersonaInputs>;
} {
  const inputResolution = resolvePersonaInputs(
    selection.inputs,
    selection.inputValues,
  ...
  return { effectiveSelection, inputResolution };
}
...
async function runInteractive(
  ...
  options: {
     ...
     * and a launch plan written so the next launch can take the fast path.
     */
    selector?: string;
  }
): Promise<number> {
  const { effectiveSelection, inputResolution } = renderEffectiveSelection(selection);
  const { personaId, harness, model, harnessSettings, systemPrompt } = effectiveSelection;
  ...
  // harnesses. For other harnesses, we instead keep installs out of the repo by
  ...
  // via claude's installRoot, or (b) open a mount. Both engage by default;
  // --install-in-repo disengages both.
  const useSessionDir =
  ...
  perfMark('runInteractive: fingerprint computed');
  ...
  perfMark('runInteractive: skill cache checked');
  ...
  // INSIDE the mount so harness skill roots, prpm.lock, etc. land in the
  // sandbox rather than the real repo. We defer it to `onBeforeLaunch`
  // below instead of pre-running here.
  ...
  perfMark('runInteractive: install block done');
  const relayMcp = resolveRelayMcpFromEnv(process.env);
  const aiMemory = resolveAiMemory(effectiveSelection.memory);
  const aiHist = aiMemory.enabled
    ? resolveAiHistConfig(process.env, aiMemory.dbPath)
  ...
  perfMark('runInteractive: harness spec + relay MCP resolved');


  // Mount branch: delegate process lifecycle (spawn, signal forwarding,
  // syncback, cleanup) to @relayfile/local-mount.
  //
  // For interactive harnesses: mount engages by default (unless
  // `--install-in-repo`). For claude this hides CLAUDE.md / .claude /
  // .mcp.json; for non-claude harnesses it routes `npx prpm install` /
  // `npx skills add` writes into the sandbox (skill plugin dir for claude
  // lives outside the mount at an absolute path, so claude still resolves
  // `--plugin-dir` normally).
  //
  ...
  if (useClean && sessionRoot) {
    ...
    // Warm-launch plan for the NEXT invocation of this selector. Built at
    // resolution time (digests must reflect the inputs this argv came from),
    // consumed at teardown if the session ends cleanly.
    const fastPlan =
      options.selector && skillCachingEnabled
        ? buildFastLaunchPlan({
            ...
            effectiveSelection,
            relayMcp,
            envResolutionValue: envResolution.value,
            inputEnv,
            resolvedMcp,
            aiHist,
            skillCacheDir,
    ...
    try {
      ...
      perfMark('mount: createMount start');
      handle = await createMount(process.cwd(), mountDir, {
        ...
        // work.
        includeGit: true,
        // Enumerate via `git ls-files` when the cwd is a git checkout: only
        // tracked + untracked-unignored files enter the mount, so gitignored
        // trees (worktrees, build outputs, caches) never get mirrored or
        // rescanned by autosync/syncBack. Falls back to the walk elsewhere.
        population: 'auto'
      });
      ...
      perfMark('mount: createMount done');
      configureGitForMount(handle.mountDir, ignoredPatterns);
      perfMark('mount: git configured');
      ...
      perfMark('mount: skills mirrored/installed');
      ...
      perfMark('mount: config files + sidecar written');
      launchMetadata = await startLaunchMetadataForLaunch(handle.mountDir);
      perfMark('mount: launch metadata started');
      if (options.capture) {
      ...
      perfMark('mount: autosync started');
      ...
      childSpawned = true;
      perfMark('spawn: harness child spawning');
      ...
      let count = 0;
      let exportedState: Record<string, FileState> | undefined;
      if (autoSync) {
        if (fastPlan && !shutdownController.signal.aborted) {
          // Full pass before stopping so the exported state also covers
          // paths only reconciles visit (`.git/**` under includeGit) —
          // without those entries, files git gc removes from the project
          // between sessions would survive as stale copies in the warm
          // mount. Re-warming is best-effort; a failure here only costs
          // the next launch its fast path.
          try {
            await autoSync.reconcile({ signal: shutdownController.signal });
          } catch {
        ...
        if (fastPlan && !shutdownController.signal.aborted) {
          try {
            exportedState = autoSync.exportState();
          } catch {
            /* re-warm skipped below */
          }
        }
        autoSync = undefined;
      }
      perfMark('exit: autosync stopped (draining reconcile done)');
      ...
      perfMark('exit: final syncBack done');

      const aborted = shutdownController.signal.aborted;
      if (fastPlan && exportedState && !aborted) {
        // The session settled cleanly: keep the mount warm for the next
        // launch instead of discarding it. cleanup()/removeSessionRoot in
        // the finally below no-op on the moved-away mount dir.
        rewarmMountAndPlan({ plan: fastPlan, mountDir: handle.mountDir, exportedState });
      }
...
async function runList(args: readonly string[]): Promise<never> {
  if (args.includes('--deployments')) {
    const { runDeploymentList } = await import('./list-command.js');
...
/**
 * Manage a session the fast path already spawned (see fast-launch.ts): the
 * harness child is running inside a claimed warm mount, and this function —
 * loaded while the harness TUI is still booting — owns everything else:
 * reattach the mount with the previous session's autosync state, reconcile
 * to catch up with repo changes made while the mount sat warm, start launch
 * metadata, and on child exit run the same drain/sync-back/re-warm flow as
 * a full launch.
 *
 * Also re-derives the harness spec from live persona resolution and compares
 * it against the plan's argv. The plan's digests should make a mismatch
 * impossible; if one slips through, the session keeps running (killing it
 * would eat the user's terminal for a stale-plan technicality) but the plan
 * is deleted and the mismatch reported so the next launch re-resolves.
 */
export async function resumeFastSession(fast: FastLaunch): Promise<never> {
  perfMark('resume: cli-impl loaded (child already running)');
  const { plan, child, sessionRoot, mountDir, statePath } = fast;
  recordRecent(plan.personaId);
  process.stderr.write(
    `→ ${plan.personaId} via ${plan.harness} (${plan.model}) [warm launch]\n`
  );

  // Child-running / syncing SIGINT semantics, mirroring runInteractive
  // (there is no pre-launch phase here — the child is already up).
  const shutdownController = new AbortController();
  let syncSpinner: Ora | undefined;
  let isSyncing = false;
  let abortPresses = 0;
  ...
  // Live resolution replay: rebuild the selection and spec, verify the plan
  // argv, and capture fresh digests for the re-warm at exit.
  let target: ResolvedTarget | undefined;
  let nextPlan: LaunchPlan | null = null;
  let launchMetadata: LaunchMetadataRun | undefined;
  const capture: RunInteractiveCapture = {
    sessionCwd: mountDir,
    harness: plan.harness as Harness,
    startedAt: fast.startedAt
  };
  try {
    target = parseSelector(plan.selector);
    const selection = buildSelection(target.spec, target.kind);
    const { effectiveSelection, inputResolution } = renderEffectiveSelection(selection);
    const { personaId, harness, model, harnessSettings, systemPrompt } = effectiveSelection;
    const skillCacheFingerprint = computeSkillCacheFingerprint({
      harness,
      skills: effectiveSelection.skills,
      repoRoot: plan.cwd
    });
    const installRoot =
      harness === 'claude' ? resolveSkillCacheDir(skillCacheFingerprint) : undefined;
    const spec = buildInteractiveSpec({
      harness,
      personaId,
      model,
    ...
    const argvMatches =
      finalArgs.length === plan.spawn.args.length &&
      finalArgs.every((arg, i) => arg === plan.spawn.args[i]) &&
      skillCacheFingerprint === plan.skillCache.fingerprint;
    if (!argvMatches) {
      deleteLaunchPlan(plan.cwd, plan.selector);
      process.stderr.write(
        'warning: cached launch plan no longer matches live persona resolution; ' +
          'this session runs with the cached spec, the next launch re-resolves.\n'
      );
    } else {
      nextPlan = buildFastLaunchPlan({
        ...
        selection,
        effectiveSelection,
        relayMcp: resolveRelayMcpFromEnv(process.env),
        envResolutionValue: undefined,
        inputEnv: inputResolution.values,
        resolvedMcp: undefined,
        aiHist: undefined,
        skillCacheDir: resolveSkillCacheDir(skillCacheFingerprint),
        skillCacheFingerprint,
        bin: spec.bin,
        finalArgs,
        ignoredPatterns: plan.mount.ignoredPatterns,
        readonlyPatterns: plan.mount.readonlyPatterns
      });
    }
    launchMetadata = await startLaunchMetadataRecording({
      selection: effectiveSelection,
      personaSpec: target.spec,
      personaSource: target.source,
      cwd: mountDir,
    ...
    capture.stampEnrichment = { ...launchMetadata.metadata };
    capture.stampingEnabled = launchMetadata.enabled;
  } catch (err) {
    process.stderr.write(
      `warning: post-spawn resolution failed (${(err as Error).message}); ` +
        'session continues, next launch takes the full path.\n'
    );
    deleteLaunchPlan(plan.cwd, plan.selector);
  }

  // Reattach the warm mount and catch up with anything that changed in the
  // repo while it sat idle. The harness is booting its TUI concurrently;
  // autosync's watchers take over from here exactly as in a full launch.
  let handle: MountHandle | undefined;
  let autoSync: AutoSyncHandle | undefined;
  try {
    const initialState = JSON.parse(readFileSync(statePath, 'utf8')) as Record<
      string,
      FileState
    >;
    handle = await attachMount(plan.cwd, mountDir, {
      ignoredPatterns: [...plan.mount.ignoredPatterns],
      readonlyPatterns: [...plan.mount.readonlyPatterns],
      excludeDirs: [],
      agentName: plan.personaId,
      includeGit: true,
      population: 'auto',
      initialState
    });
    ...
    perfMark('resume: mount reattached + reconciled');
  } catch (err) {
    process.stderr.write(
      `warning: could not reattach warm mount (${(err as Error).message}). ` +
        `Session changes will NOT sync back automatically; they remain in ${mountDir}.\n`
    );
    deleteLaunchPlan(plan.cwd, plan.selector);
    nextPlan = null;
  }
  ...
  try {
    if (autoSync) {
      if (nextPlan && !shutdownController.signal.aborted) {
        try {
          await autoSync.reconcile({ signal: shutdownController.signal });
        } catch {
          /* fall through to the normal drain */
    ...
    const aborted = shutdownController.signal.aborted;
    if (handle && nextPlan && exportedState && !aborted) {
      rewarmMountAndPlan({ plan: nextPlan, mountDir: handle.mountDir, exportedState });
    }
    const qualifier = aborted ? ' (partial)' : '';
    syncSpinner.succeed(
      count > 0
...
export async function main(): Promise<void> {
  perfMark('main() entered (module graph loaded)');
  ...
  if (subcommand === 'deploy') {
    const { runDeploy } = await import('./deploy-command.js');
  ...
  if (subcommand === 'runs') {
    const { runRuns } = await import('./runs-command.js');
    await runRuns(rest);
    return;
  }
  ...
  if (subcommand === 'deployments') {
    ...
    if (action === 'logs') {
      const { runDeploymentLogs } = await import('./list-command.js');
  ...
  if (subcommand === 'destroy') {
    const { runDestroy } = await import('./destroy-command.js');
    await runDestroy(rest);
    return;
  }

  if (subcommand === 'login') {
    const { runLogin } = await import('./deploy-command.js');
    await runLogin(rest);
    return;
  }

  if (subcommand === 'logout') {
    const { runLogout } = await import('./deploy-command.js');
