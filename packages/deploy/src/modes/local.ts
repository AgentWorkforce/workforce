...
/**
 * Local-mode launcher. Spawns `node <bundle.runnerPath>` as a child
 ...
 * so Ctrl-C in `--mode local` produces an orderly shutdown.
 */
export const localLauncher: ModeLauncher = {
  async launch(input: ModeLaunchInput): Promise<ModeLaunchHandle> {
    ...
    // The generated runner imports `@agentworkforce/runtime`. In local mode
    ...
    // user pipes into `workforce deploy --mode local` flow straight into
    ...
    if (child.pid === undefined) {
      throw new Error('local launcher: failed to spawn runner (no pid assigned)');
    }
    ...
    // Bridge parent-process signal handlers so Ctrl-C in --mode local
