...
- **NEVER STOP.** This is the prime directive of an autonomous run. The agent always has a next action or an armed wake-up. It never idles waiting for permission it already holds, and never ends a run with open work and no scheduled continuation. "Blocked" is not a stopping state: if the current lane is blocked, the agent self-unblocks by spawning support, running diagnostics, working a parallel lane, or arming a wake-up against the specific signal it is waiting on. The run ends only when the deliverable is met or the operator says stop. Continuity lives in the next lead or worker, never in a halted session. A stalled overnight run is a contract failure, not a neutral pause. Corollary: **do not offload to the operator a check or action the agent can perform itself** via CLI, source, `gh`, or a spawned worker.
- **Cross-repo work needs no separate approval.** The §2 grants (auto-merge, flip, swarm-blockers, rollback) and the run's authority extend to every AgentWorkforce repo the deliverable touches, not just the primary repo named in §1. When the fix lives in a sibling repo (`workforce`, `relayfile`, `relayfile-adapters`, `relayauth`, `agents`, `relaycast`, etc.), the agent implements, opens, reviews, and, within the §3 bar, merges there under the same grant. It does not pause to re-ask the operator for permission to act in a different repo. The implementation leg of a cross-repo fix is never operator-gated; only standing constraints and explicit publish/deploy gates still apply. Treating a cross-repo dependency as a fresh approval boundary is a common autonomous-run stall.
